...
def render_powerflow(data: dict) -> str:
    ...
    return f"""
    <div class="powerflow-mini" aria-label="Energy flow summary">
      <div class="mini-node"><span>Solar</span><b>{solar:.1f} kW</b></div>
      <div class="mini-node"><span>Home</span><b>{house:.1f} kW</b></div>
      <div class="mini-node"><span>Battery</span><b>{battery:.1f} kW</b></div>
      <div class="mini-node"><span>Grid</span><b>{grid:.1f} kW</b></div>
...
def powerflow_hero_component(data: dict) -> dict:
    return {
        ...
        "decision": data.get("decision", "Observing energy flow"),
        "updated": data.get("last_update", "latest cycle"),
    }
